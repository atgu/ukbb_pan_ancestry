...
def get_clumping_results_path(
    pop: str = "full", high_quality: bool = False, not_pop: bool = True, max_pops: bool = False
):
    """
    ...
    mt_name = ("max_pops" if max_pops else ("not_" if not_pop else "")) + (
        f"{pop}" if pop in POPS else ("full" if pop == "full" and not max_pops else "")
    )
...
def get_clump_sumstats_col_ht_path(high_quality: bool = True, pop: str = "full"):
    return (
        f'{public_bucket}/misc/prs/clumped_sumstats{"" if high_quality else "_raw"}{f"_{pop}" if pop else ""}.cols.ht'
    )


def get_clump_sumstats_row_ht_path(high_quality: bool = True, pop: str = "full"):
    return (
        f'{public_bucket}/misc/prs/clumped_sumstats{"" if high_quality else "_raw"}{f"_{pop}" if pop else ""}.rows.ht'
    )
...
def get_munged_otg_v2d_path(release: str = otg_release, ukbb_exclusion: str = "NEALE_SAIGE", extension: str = "ht"):
    if ukbb_exclusion == "NEALE_SAIGE":
        suffix = ""
    elif ukbb_exclusion == "":
        suffix = "_no_exclusion"
    else:
        suffix = f"_no_{ukbb_exclusion}"
    return f"{bucket}/misc/otg/{release}/munged_otg_v2d{suffix}.{extension}"


def get_munged_round2_path(extension: str = "ht"):
    return f"{bucket}/misc/ukbb_round2/munged_ukbb_round2.{extension}"


def get_known_ukbb_loci_path(
    pop: str, release: str = otg_release, ukbb_exclusion: str = "NEALE_SAIGE", extension: str = "ht"
):
    if ukbb_exclusion == "NEALE_SAIGE":
        suffix = ""
    elif ukbb_exclusion == "":
        ...
        suffix = f"_no_{ukbb_exclusion}"
    return f"{bucket}/misc/otg/{release}/known_ukbb_loci{suffix}.{pop}.{extension}"
